...
public class SevenSidedDieTests {

    private Logger logger = LoggerFactory.getLogger(SevenSidedDieTests.class);
    ...
    public void test1(int retries) {
        ...
        for (int i = 0; i < retries; i++) {
            randomNumberOccurrences.compute(SevenSidedDie.execute(), (k, v) -> v == null ? 1 : ++v);
        }

        int exactDistributionOccurrence = retries / 7;
        /**
    ...
    private static IntStream dataProvider() {
        ...
        return IntStream.range(0, 10).map(val -> 1000 + random.nextInt(14_000));
    }
